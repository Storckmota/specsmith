...
- Test Writer uses `===METADATA=== / ===CODE=== / ===END===` delimiters so generated source code is never subject to JSON string escaping. If a real model returns output that does not match this contract, the agent retries once — passing the bad output back and asking for a reformat. The retry is Test Writer only; the rest of the pipeline is not re-run. Validated against User Auth OpenAPI with gpt-4o-mini: score 95/100, no retry needed.
